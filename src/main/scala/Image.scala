...
object Image {
  def saveImage(solution: Solution, file: File): Try[Unit] = Try {
    val bufferedImage = new BufferedImage(1920, 1080, BufferedImage.TYPE_INT_RGB)
    val g = bufferedImage.getGraphics

    val groupCount =  solution.candidate.activitiesByGroup.size

    for {
      ((group, activities), groupIndex) <- solution.candidate.activitiesByGroup.toSeq.sortBy(_._1.number) .zipWithIndex
      minX = 0
  ...
  def drawGroup(g: Graphics,
    ...
    for (activity <- activities) {
      ...
      if (!activity.pause) {
        ...
        g.setFont(bigFont)
        g.drawString(activity.asString(Time.minutesLater(startTime, startInMinutes)), activityFromX + 10, (minY + maxY) / 2)
      }
